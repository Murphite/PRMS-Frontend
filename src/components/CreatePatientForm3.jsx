...
const CreatePatientForm3 = ({ formData, setFormData, setStage }) => {
    ...
    const [selectCount, setSelectCount] = useState(flag);
    const [countFlags, setCountFlag] = useState([]);
    async function getCountFlags() {
    ...
    const [contin, setContinue] = useState(false);
    const [toggleOn, setToggleOn] = useState(false);
    const navigate = useNavigate();

    function nextForm() {
        formData.otherCountry = otherCountry;
    }
        ...
        <div className=" bg-white h-[70rem]  relative  ">
            ...
            <div className=" absolute   left-[35rem] h-[65rem] w-[32rem] mt-10">
                <div className=" relative bg-teal-100 bg-opacity-50 w-full rounded-lg  h-[3rem] mt-3 border border-solid border-teal-600 focus:outline-none px-5">
                    <label className=" text-teal-500 block">Name</label>
                    <input
                ...
                <div className=" relative bg-teal-100 bg-opacity-50 w-full rounded-lg  h-[3rem] mt-3 border border-solid border-teal-600 focus:outline-none px-5">
                    <label className=" text-teal-500 block">Relationship</label>
                    <input
                        className=" bg-transparent focus:outline-none w-full "
                                    ...
                                    countFlags.map((flag, i) => (
                                        <div key={i}>
                                            <img
                                                ...
                                                className="w-[4rem] h-[50px] my-2 cursor-pointer"
                                                onChange={setFormData}
                                                value={
                ...
                <p className=" mt-4"> CONSENT AND AGREEMENT</p>
                <Checkbox
                    onClick={() => setContinue(!contin)}
                    className=" mt-4  relative "
                >
                    I consent to the use of my health information for the
                    purposes outlined in the app's privacy policy.{" "}
                    {contin ? "" : ""}
                </Checkbox>
                <Checkbox
                ...
                <a
                    href="#"
                    className=" text-teal-500  ml-44 pt-12 top-7 relative italic"
                >
                    Terms & Condition
                </a>
                <div>
                    <a
                        href="#"
                        className=" text-teal-500 ml-48 pt-12 top-9 relative italic"
                    >
                        Privacy policy
                    </a>
                </div>
                ...
                {contin && toggleOn && (
                    <div>
                        <button
                            className="bg-teal-500 w-[28rem] rounded-full h-10 mt-20  ml-8 text-white italic"
                            type="submit"
                            onClick={() => nextForm()}
                        >
                            I agree with this
